...
Users that wish to prevent this behavior (not recommended) can pass `-backupzdev=0` at the command-line when starting the client, or add `backupzdev=0` to their `deviant.conf` file.
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the Deviant address as it's first parameter and the passphrase as it's second parameter.
