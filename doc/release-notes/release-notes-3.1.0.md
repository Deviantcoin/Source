Deviant Core version *3.1.0* is now available from:  <https://github.com/deviant-project/deviant/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/deviant-project/deviant/issues>
...
zDEV Staking is here! zDEV staking will be activated on the morning of the 8th of May 2018. With the release of zDEV staking, there are effectively 2 versions of zDEV, zDEV minted on the 3.0.6 Deviant wallet or lower, and zDEV minted on Deviant wallet version or higher. New features in this release will require the use of zDEV v2, zDEV minted on this wallet release 3.1.0 or later. If you currently hold zDEV v1 and wish to take advantage of zDEV staking and deterministic zDEV, you will need to spend the zDEV v1 to yourself and remint zDEV v2.
...
To encourage the use of zDEV and increase the Deviant zerocoin anonymity set, the Deviant payment schedule has been changed to the following:
...
The visual layout of the Deviant Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zDEV minting feature. This is enabled by default and the enablezeromint=0 setting in the deviant.conf file will overwrite the GUI option.
...
### Summary
 ...
 The Deviant wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the deviant.conf file, found in the Deviant data directory.
...
Deviant Daemon & Client (RPC Changes)
--------------
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Deviant support group, which often times sees users say "I didn't spend that zDEV". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Deviant masternodes, the monthly budget available to be utilised has been increased to 42,000 DEV / month. This DEV only has the opportunity to be raised once per month (paid to winning proposals) with any unused DEV not created by the blockchain.

### Budget Finalization Fee

The Deviant finalization fee for successful proposals has now been reduced, this fee is now 5 DEV down from 50 DEV. The total fee outlay for a successful proposal is now a total of 55 DEV.
...
Several critical security flaws in the zerocoin protocol and Deviant's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zDEV Control dialog in the QT wallet, a user is able to see zDEV marked as version 1 or 2.
...
Deviant's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Deviant used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Deviant using the bug bounty program. The modulus is now correctly imported and Deviant's accumulators have been changed to use the new proper modulus.
