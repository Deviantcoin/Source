...
*Setup instructions for a gitian build of Deviant using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Deviant
...
- [Building Deviant](#building-deviant)
...
Clone the git repositories for deviant and gitian and then checkout the deviant version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the deviant repository
...
To build Deviant (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the deviant repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/deviant/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/deviant-crypto/deviant
...
For example:
```bash
URL=https://github.com/crowning-/deviant.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit Deviant=${COMMIT} --url deviant=${URL} ../Deviant/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Deviant=${COMMIT} --url deviant=${URL} ../Deviant/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Deviant=${COMMIT} --url deviant=${URL} ../Deviant/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/deviant-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/deviant-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/deviant-build.assert
```
...
[deviant/gitian.sigs](https://github.com/deviant-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
