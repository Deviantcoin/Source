...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "deviant" is a composite category enabling all Deviant-related debug output
            if (ptrCategory->count(string("deviant"))) {
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DeviantCore";
#else
    fs::path pathRet;
